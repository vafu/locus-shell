...
watched GTK properties. Locus graph-derived sources receive updates through locusfs watches; custom sources use the same Observable binding path.

```text
+---------------+                    +------------------+                    +-----------------+
|  locusfs      | --file watch/read->| RxRust Observable| --Injected Msg---->|   Relm4 Model   |
| (FUSE Graph)  |                    | source expression|                    | (State Mutate)  |
+---------------+                    +------------------+                    +-----------------+
                                                                                      |
                                                                               Triggers #[watch]
                                                                                      |
...
The client should not poll or diff large graph payloads. Locusfs owns graph exposure and invalidation; the shell Observable layer composes updates delivered by source functions.
...
- Support binding sources:
  - Locus graph property and collection expressions through observable source
    functions.
  - Pure D-Bus property expressions through typed D-Bus/common service helpers.
