        ...
        P: Provider<NodeId>,
    {
        ...
        fn stream(self, cancellation: CancellationToken) -> Self::Stream {
            Box::pin(async_stream::stream! {
                ...
                Some(token) = node_cancellation.take() { token.cancel(); } let token
                = cancellation.child_token(); let binding = match self.direction {
                CollectionDirection::Sources => { NodeListBinding:: < Target >
                ::sources(owner, self.relation) } CollectionDirection::Targets => {
                NodeListBinding:: < Target > ::targets(owner, self.relation) } }; let
                stream : Pin < Box < dyn Stream < Item = Result < Vec < NodeId >,
                locus_provider::WatchError >> + Send >> = match self.kind {
                Some(kind) => Box::pin(binding.filter_kind(kind).stream(token
                .clone())), None => Box::pin(binding.stream(token.clone())), }; nodes
