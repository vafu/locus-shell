...
- Investigated PipeWire lag: locusfs `plugins/pipewire` debounces `pactl subscribe` bursts for 100 ms and publishes one full snapshot, so rapid intermediate volume states are collapsed before shell observables receive them.
...
- Added a bar-side MPRIS widget source for the intended `/mpris/player/*` locusfs contract and documented that locusfs still needs a live MPRIS projection because the generic D-Bus plugin does not subscribe to property changes.
