...
use crate::sources::{WindowNode, window_app_id, window_title};

use super::{WindowTileKind, WindowTileView};

pub(crate) fn window_tile_for_window(window: WindowNode) -> Observable<WindowTileView> {
    let title = window_title(window.clone());
    let app_id = window_app_id(window);

    shell_core::source::into_observable::<String, _>(title)
        .combine_latest(app_id, |title, app_id| WindowTileView {
            kind: window_kind(&title),
            icon: icon_for_app_id(&app_id),
            tooltip: title,
            urgent: false,
            context_pct: 0,
            substatus_count: 0,
