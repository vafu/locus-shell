...
- Keep helper structs, source composition, path construction, decoding policy,
  and formatting helpers private in the same widget module unless another
  widget actually needs them.
...
- Consumer sources must depend on shell-core observable helpers, not the
  `locusfs-watch` client API. Do not call `locusfs_watch::read_*`,
  `Watch::open`, or Rx future/stream factories from `rsynapse-shell`; request
  or add a shell-core observable primitive instead.
